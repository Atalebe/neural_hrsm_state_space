...
The lag-ablation curves suggest that the memory contribution is not exhausted by the nearest previous bin. Instead of a purely first-order pattern in which lag 1 captures nearly all predictive leverage, controlled memory generally increases as additional recent lags are included. For active-unit recruitment, the cross-session median controlled gain rises from approximately \(0.046\) under lag 1 to \(0.087\) under lags \(1\)--\(4\). This supports an operational interpretation in which spontaneous population history is distributed across a short recent temporal window rather than being reducible to a single immediately preceding state.

Population-state speed remains the weakest target across the main target synthesis and the lag-ablation audit. This weak support is informative. State speed is a derivative-like quantity describing how rapidly the population state moves, whereas the present memory model uses recent population-position features. The result is therefore consistent with the interpretation that the retained-history signal is primarily structural and organizational, not kinetic. A stronger test of velocity or acceleration memory would require models that explicitly include velocity-like or acceleration-like predictors rather than position-lag predictors alone.
